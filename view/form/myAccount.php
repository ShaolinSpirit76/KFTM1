      ...
      <fieldset>
          ...
          <h5 for="Identity" class="card-title font-weight-bolder text-white"><strong>1. Identité</strong><br /><br /></h5>
              ...
              <label class="text-white"><i>Modifier : </i></label> <input class="inputInscription <?php echo (isset($_POST['birthDate']) && !preg_match($regexDate, date('d/m/Y',strtotime($_POST['birthDate']) )))? 'red':'';  ?>" value="<?= $_POST['birthDate']?>" type="date" name="birthDate" id="birthDate" placeholder="jj/mm/aaaa"  /><p class="errorMessage"><?= (isset($error['errorBirthDate'])) ? $error['errorBirthDate'] : ''; ?></p>
                 ...
                 <li class="font-weight-bolder text-white"><label for="mail">Adresse mail : </label> <input class="inputInscription <?php echo (isset($_POST['mail']) && !preg_match($regexMail, $_POST['mail']))? 'red':'';  ?>" type="text" id="mail" name="mail" value="<?= ($_SESSION['userInfos'][0]['mail']) ?>" placeholder="julie.dupont@exemple.com" required /><p class="errorMessage"><?= (isset($error['errorMail'])) ? $error['errorMail'] : ''; ?></p><p class="errorMessage"><?= (isset($error['errorMailChecking'])) ? $error['errorMailChecking'] : ''; ?></p></li>


                 <li class="font-weight-bolder text-white"><label for="phoneNumber">Numéro de téléphone : </label> <input class="inputInscription <?php echo (isset($_POST['phoneNumber']) && !preg_match($regexPhone, $_POST['phoneNumber']))? 'red':'';  ?>" value="<?= ($_SESSION['userInfos'][0]['phoneNumber']) ?>" type="tel" id="phoneNumber" name="phoneNumber" placeholder=" 06.xx.xx.xx.xx " /><p class="errorMessage"><?= (isset($error['errorPhone'])) ? $error['errorPhone'] : ''; ?></p></li>
...
<fieldset>
            ...
            <li class="font-weight-bolder text-white"><label for="userLog"><I>Identifiant : </I> <?= ($_SESSION['userInfos'][0]['userLog']) ?></label></li> 
            ...
            <span class="font-weight-bolder text-white"><label for="passwordConnect">Mot de passe actuel  </label><br /> <input class="<?php echo (isset($_POST['passwordConnect']) && !preg_match($regexPassword, $_POST['passwordConnect']))? 'red':'';  ?>" value="<?= $_POST['passwordConnect']?>" type="password" name="passwordConnect" id="passwordConnect" placeholder="Mot de passe" /> <small class="text-white"><br /><i>Renseigne ton mot de passe pour confirmer l'action.</i></small><p class="errorMessage"><?= (isset($error['errorPassword'])) ? $error['errorPassword'] : ''; ?> </p><p class="errorMessage"><?= (isset($error['errorCheckPassword'])) ? $error['errorCheckPassword'] : ''; ?></p></span> 

            <span class="font-weight-bolder text-white"><label for="userLog">Nouvel identifiant </label><br />
            <input class="inputInscription <?php echo (isset($_POST['userLog']) && !preg_match($regexLogin, $_POST['userLog']))? 'red':'';  ?>" value="<?= $_POST['userLog']?>" type="text" name="userLog" id="userLog" placeholder="Pseudo" /><small class="text-white"><br /><i>Vous pouvez tout simplement choisir votre adresse mail.</i></small>
            <p class="errorMessage"><?= (isset($error['errorLogin'])) ? $error['errorLogin'] : ''; ?></p></span>
...
<input class="<?php echo (isset($_POST['passwordConnect']) && !preg_match($regexPassword, $_POST['passwordConnect']))? 'red':'';  ?>" value="<?= $_POST['passwordConnect']?>" type="password" name="passwordConnect" id="passwordConnect" placeholder="Mot de passe" /> <small class="text-white"><br /><i>Renseigne ton mot de passe pour confirmer l'action.</i></small><p class="errorMessage"><?= (isset($error['errorPassword'])) ? $error['errorPassword'] : ''; ?> </p><p class="errorMessage"><?= (isset($error['errorCheckPassword'])) ? $error['errorCheckPassword'] : ''; ?></p></span> 

                

<span class="font-weight-bolder text-white"><label for="passwordConnect">Nouveau mot de passe </label><br />
<input class="inputInscription <?php echo (isset($_POST['password']) && !preg_match($regexPassword, $_POST['password']))? 'red':'';  ?>" value="<?= $_POST['password']?>" type="password" name="password" id="password" /><p class="errorMessage"><?= (isset($error['errorPassword'])) ? $error['errorPassword'] : ''; ?></p></span>

              
<span class="font-weight-bolder text-white"><label for="confirmPassword"><i>Confirmation * </i></label><br /> 
<input class="inputInscription <?php echo (isset($_POST['confirmPassword']) && !preg_match($regexPassword, $_POST['confirmPassword']))? 'red':'';  ?>" value="<?= $_POST['confirmPassword']?>" type="password" name="confirmPassword" id="confirmPassword" />
    <p class="card-text"><small class="text-white"><i>Entre 8 et 15 caractères, contenant au moins une minuscule et une majuscule, un chiffre et un caractère spécial.</i></small></p><p class="errorMessage"><?= (isset($error['errorConfirmPassword'])) ? $error['errorConfirmPassword'] : ''; ?></p></span>
...
<li class="font-weight-bolder text-white"><label for="status"><i>Rang : </i> <?= ($_SESSION['userInfos'][0]['status']) ?></label> <input type="radio" id="<?= ($_SESSION['userInfos'][0]['status']) ?>" name="status" value="<?= ($_SESSION['userInfos'][0]['status']) ?>"> <small class="text-white"><i>Clique pour afficher et modifier</i></small></li>
<?php endif; ?>
...
<li class="font-weight-bolder text-white"><label for="studentCourse"><i>Discipline (élève) : </i> <?= ($_SESSION['userInfos'][0]['studentCourse']) ?></label></li>
<?php endif; ?>

            <li class="font-weight-bolder text-white"><label for="studentCourse">Modifier : </label>
...
<li class="font-weight-bolder text-white"><label for="teacherCourse"><i>Cours (maître) : </i> <?= ($_SESSION['userInfos'][0]['teacherCourse']) ?></label></li>
<?php endif; ?>

            <li class="font-weight-bolder text-white"><label for="teacherCourse">Modifier : </label>
...
<li class="font-weight-bolder text-white"><label for="studentYear">Modifier : </label>
...
<?php endif; ?>
          ...
          <li class="font-weight-bolder text-white"><label for="childBelt">Modifier : </label>
...
<?php endif; ?>
          ...
          <li class="font-weight-bolder text-white"><label for="studentBelt">Modifier : </label>
...
<?php endif; ?>
          ...
          <li class="font-weight-bolder text-white"><label for="teacherRank">Modifier : </label>
...
<?php if (isset($_SESSION['userInfos'][0]['presentation'])): ?>
...
<p class="font-weight-bolder text-white text-justify"><label for="presentation"><br /><br />Modifier ta présentation</label></p> <!--Pour le maxlenght du textarea, il ne commence qu'à 18 caractères. Il faut donc mettre le nombre souhaité +18-->
...
<p><br /><button id="modifRequest" type="submit" name="button" class="float-right rounded">Enregistrer les modifications</button></p>
